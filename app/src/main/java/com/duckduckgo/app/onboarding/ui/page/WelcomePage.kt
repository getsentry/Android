...
class WelcomePage : OnboardingPageFragment(R.layout.content_onboarding_welcome) {
    ...
    private fun configureDaxCta() {
        context?.let {
            ...
            binding.ctas!!.hiddenTextCta.text = ctaText.html(it)
            binding.ctas!!.dialogTextCta.textInDialog = ctaText.html(it)
            binding.ctas!!.dialogTextCta.setTextColor(ContextCompat.getColor(it, R.color.grayishBrown))
            binding.ctas!!.cardView.backgroundTintList = ContextCompat.getColorStateList(it, R.color.white)
        }
        binding.ctas!!.triangle.setImageResource(R.drawable.ic_triangle_bubble_white)
    }

    private fun setSkipAnimationListener() {
        binding.longDescriptionContainer.setOnClickListener {
            if (binding.ctas!!.dialogTextCta.hasAnimationStarted()) {
    ...
    private fun scheduleWelcomeAnimation(startDelay: Long = ANIMATION_DELAY) {
        welcomeAnimation = ViewCompat.animate(binding.welcomeContent as View)
            ...
            .withEndAction {
                typingAnimation = ViewCompat.animate(binding.ctas!!.daxCtaContainer)
                    .alpha(MAX_ALPHA)
                    .setDuration(ANIMATION_DURATION)
                    .withEndAction {
                        welcomeAnimationFinished = true
                        binding.ctas!!.dialogTextCta.startTypingAnimation(ctaText)
    ...
    private fun finishTypingAnimation() {
        welcomeAnimation?.cancel()
        binding.ctas!!.dialogTextCta.finishAnimation()
        setPrimaryCtaListenerAfterWelcomeAlphaAnimation()
    }

    private fun setPrimaryCtaListenerAfterWelcomeAlphaAnimation() {
        binding.ctas!!.primaryCta.setOnClickListener { event(WelcomePageView.Event.OnPrimaryCtaClicked) }
    }
